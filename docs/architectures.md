...
| [State-Space (S4-lite)]({{ site.baseurl }}/architectures/state-space) | O(1) per-step state, any length | Diagonal linear-recurrent sequence model (LTI + input-gated "selective"); streaming step() matches the full-sequence pass byte-for-byte |
| [Decision Trees & GBDT]({{ site.baseurl }}/architectures/decision-trees) | int8 compare + branch, no MACs | Single trees + gradient-boosted ensembles for tabular data; quantized thresholds preserve splits exactly |
| [Neuro-Fuzzy (ANFIS)]({{ site.baseurl }}/architectures/anfis) | 448 bytes (16-rule Q16.16) | Takagi-Sugeno rules learned from data, with per-rule firing strengths readable back after every inference |
