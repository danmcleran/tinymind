...
**Pruning an oversized grid.** Three membership functions per input is 81 rules and 405 consequent parameters against 500 training samples — it overfits catastrophically (train RMSE 0.000116, test RMSE **1.77**). Pruning by mean firing strength recovers it: 25 of 81 rules gives test RMSE 0.00533, a ~330x improvement while shipping 31% of the rule base. This is why `cpp/anfis.hpp` carries an explicit rule table rather than an implicit `M^N` grid.
