...
`unit_test/embedded` is the regression guard — its Makefile builds eight corners of the `(FLOAT, STD, QUANT, FP16, INT16_ACCUM, SIMD_*)` matrix (`freestanding`, `no_stdlib`, `no_fpu`, `hosted`, `quant_freestanding`, `fp16_freestanding`, `int16_accum_freestanding`, `simd_disabled`) and `make check` runs all of them. A separate `simd_prereq_regressions` make target locks the static_assert prerequisite chain via compile-failure checks. Three further embedded gates live in the same Makefile: `arm_crosscompile` (HARD — build-only smoke compile across Cortex-M0+/M4/M33 and soft/hard float ABIs with `arm-none-eabi-g++`, proving the headers assemble for a real MCU, not just the host), `arm_crosscompile_clang` (HARD — the same seven corners cross-compiled with `clang++ --target=arm-none-eabi`, borrowing the arm-none-eabi GCC's target C++ headers via a queried `-isystem` list rather than hardcoded paths; covers the LLVM embedded toolchain for Arm deployment path), `heapfree` (HARD — `nm`-asserts the freestanding + quant objects reference no `malloc`/`operator new`), and `stack_usage` (advisory — `-fstack-usage` report of the deepest per-frame stack for the deployable Cortex-M4 int8 shape). Two more g++-only checks hang off the top-level `make analyze`: `header-selfcheck` (HARD — each `cpp/**/*.hpp` compiled standalone behind a `tinymind_platform.hpp` prelude; a header that only builds via bundled include order fails, so every header carries its own `<cstddef>`/`<cstdint>` includes) and `warnings-strict` (advisory — `-Wconversion`/`-Wsign-conversion`/`-Wdouble-promotion` over the fixed-point paths). Two top-level clang targets cover second-toolchain conformance: `check-clang` (HARD — the embedded gate matrix plus the `CLANG_SUITES` Boost suites built and run with `clang++`) and `check-clang-avx2` (HARD — the `TSAN_SUITES` dispatch consumers with the AVX2 backend compiled in). `CLANG_SUITES` omits only `unit_test/integration` (it shells out to already-built example binaries, so it exits 127 on a clean tree); `check-clang-integration` (HARD) covers it instead by building the twelve exemplars it consumes with `clang++` first, which also proves the int8 pipelines are bit-exact across compilers since the goldens are shared. `check-clang-examples` (HARD) builds every example under `clang++` — the list is discovered from the tree rather than enumerated, so a new example is covered when it is added; it is a regression guard, since the two examples that were failing the clang build (`maze`, `dqn_maze`) were fixed in #174. `check-msan` (HARD) runs the embedded gate corners under MemorySanitizer, closing the uninitialized-read gap that ASan and UBSan structurally cannot cover — it is scoped to `unit_test/embedded` because MSan needs every linked object instrumented (a stock libstdc++ makes the Boost suites die inside Boost.Test's static init), and it runs at `-O0` because at `-O1`+ the optimizer folds undef reads away before instrumentation sees them. **A new sanitizer or CI gate must be shown to fail before it is trusted** — plant the defect it exists to catch and confirm it is reported. That `-O0` is why this rule is written down rather than assumed: at `-O1` a planted uninitialized read is optimized away and the job passes while detecting nothing, so raising the level to tidy up build flags would leave the gate green and blind. Sub-Makefiles assign `CC=g++`, but a command-line override beats a makefile assignment and propagates through `MAKEFLAGS`, so `make CC=clang++ ...` needs no sub-Makefile edits. Warning names that exist in only one compiler must use `TINYMIND_DISABLE_WARNING_GCC_ONLY` / `_CLANG_ONLY` from `include/compiler.h` — the plain `TINYMIND_DISABLE_WARNING` form errors on the compiler that does not own the name (`-Wunknown-warning-option` under `-Werror`). Float-typed Adam/RMSprop/Xavier require both `FLOAT && STD`. When adding a new header to `cpp/`, include `"include/tinymind_platform.hpp"` first and gate any `<cmath>`/`<type_traits>`/`std::` use; for SFINAE on `is_floating_point`, use `tinymind::enable_if` and `tinymind::is_floating_point` from `cpp/include/tinymind_traits.hpp` so the layer compiles at `STD=0`.
...
- **`unit_test/`** — `nn/` (NN correctness), `qformat/` (compile-time `static_assert`), `qlearn/`, `quantization/` (int8 path), `embedded/` (cross-corner regression matrix + `simd_prereq_regressions`), `integration/` (Phase 16 golden-byte exemplar suite; int8 exemplars plus the Q16.16 ANFIS one), `pinn/`, `ltc/`, `cfc/`.
...
- **`apps/`** — `activation/` (LUT generator for `lookupTables.cpp`), `import_pytorch/` + `import_onnx/` (Phase 15 int8 importers), `anfis_train/` (host ANFIS trainer: hybrid least-squares + normalized premise descent, grid or subtractive-clustering scatter partition, bell/triangular membership policies, rule pruning; emits the frozen arrays `cpp/anfis.hpp` consumes; numpy only, never run in CI).
