...
- **`ltc/`** — Boost.Test suite for the `LtcCell` fused ODE solver (`cpp/ltc.hpp`): closed-form fixed-point parity (`dt`-invariant steady state), reverse-mode (`RevVar`) adjoint through the cell vs central finite differences, forward-mode (`MultiDual`) vs reverse-mode value+gradient agreement, and trainability via `pinn::sgdStepReverse`. Builds with `TINYMIND_ENABLE_FLOAT=1 STD=1` and `TINYMIND_LTC_REVERSE_TRAINING=1`.
- **`cfc/`** — Boost.Test suite for the `CfCCell` (`cpp/cfc.hpp`): time-gated interpolation range bound, reverse-mode adjoint vs finite differences, forward/reverse-mode value+gradient agreement, and trainability via `pinn::sgdStepReverse`. (The int8 `QCfCCell` parity test lives in `unit_test/quantization`.)
